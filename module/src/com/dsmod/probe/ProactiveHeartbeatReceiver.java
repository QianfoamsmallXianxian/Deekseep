...
public final class ProactiveHeartbeatReceiver extends BroadcastReceiver {
    ...
    private static final String V241_CHANNEL_ID = "deekseep_proactive_messages_v241";
    private static final String V241_REPLY_CHANNEL_ID = "deekseep_reply_ready_v241";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_BOOT_COMPLETED.equals(action)
            ...
            // Package replacement kills the foreground service while preserving its requested
            // state. Restore it immediately or the target listener remains bound but frozen.
            if (z21.isRequested(context)) z21.setEnabled(context, true);
    ...
    private static void postMessageNotification(Context context, String message,
        ...
        final boolean v241 = isTargetV241(context);
        final String channelId = v241 ? V241_CHANNEL_ID : CHANNEL_ID;
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel channel = new NotificationChannel(channelId,
                    UiLanguage.text(context, "DeepSeek 主动消息", "DeepSeek proactive messages"),
                    v241 ? NotificationManager.IMPORTANCE_HIGH
        ...
        try {
            ...
            Log.i(TAG, "proactive notification posted channel=" + channelId
                    + " scope=" + normalizeConversationId(conversationId));
        } catch (Throwable t) {
    ...
    private static void postReplyReadyNotification(Context context,
        ...
        if (manager == null) return;
        final boolean v241 = isTargetV241(context);
        final String channelId = v241 ? V241_REPLY_CHANNEL_ID : REPLY_CHANNEL_ID;
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel channel = new NotificationChannel(channelId,
        ...
        try {
            ...
            Log.i(TAG, "reply-ready notification posted channel=" + channelId
    ...
    /** Host-version gate persisted by the injected 2.4.1 activation heartbeat. */
    private static boolean isTargetV241(Context context) {
        if (context == null) return false;
        SharedPreferences activation = context.getSharedPreferences(
                "deekseep_activation", Context.MODE_PRIVATE);
        return activation.getLong("target_version_code", -1L) == 257L
                && "2.4.1".equals(activation.getString(
                "target_version_name", ""));
    }
